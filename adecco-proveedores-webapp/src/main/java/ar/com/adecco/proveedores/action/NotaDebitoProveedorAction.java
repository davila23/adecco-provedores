...
@Named
@ViewScoped
public class NotaDebitoProveedorAction implements Serializable
{
    ...
    public NotaDebitoProveedorAction() {
    ...
    @PostConstruct
    public void init() {
        //this.facturas = (LazyDataModel)new NotaDebitoProveedorAction$1(this);
    }
    ...
    public boolean isNew() {
        return this.notaDebito.getId() == 0L;
    }
    
    public void alta() {
        this.setVista(Vista.ALTA_ND);
        this.facturasDataTable = (DataTable)FacesContext.getCurrentInstance().getViewRoot().findComponent("admNotaDebitoForm:facturasDataTable");
    ...
    public void guardar() {
        try {
            if (this.actualizar()) {
                this.load(this.notaDebito.getId());
                Mensajes.displayFacesMessage(FacesMessage.SEVERITY_INFO, "La nota de d\u00e9bito se guardo correctamente.");
            }
        }
        catch (ServiceException ex) {
            final Iterator<String> iterator = ex.getMessages().iterator();
    ...
    public void administrar(final Perfil perfilUsuario, final NotaDebito notaDebito) {
        this.setPerfilUsuario(perfilUsuario);
        this.setVista(Vista.EDICION_ND);
        System.out.println("administrar: Id ND: " + notaDebito.getId());
        try {
            this.load(notaDebito.getId());
        }
        catch (ServiceException ex) {
            final Iterator<String> iterator = ex.getMessages().iterator();
            while (iterator.hasNext()) {
    ...
    public void obtenerListaImpuestos() throws ServiceException {
        ...
        if (this.notaDebito != null) {
            this.tipoImpuestos.addAll(this.impuestoService.getImpuestosByType(this.notaDebito.getCompania(), "IVA"));
            this.tipoImpuestos.addAll(this.impuestoService.getImpuestosByType(this.notaDebito.getCompania(), "PERCIIBB"));
            this.tipoImpuestos.addAll(this.impuestoService.getImpuestosByType(this.notaDebito.getCompania(), "OTHPERC"));
        }
    }
    
    public void obtenerNotaDebito(final long n) throws ServiceException {
        this.notaDebito = this.notaDebitoService.getById(n, true);
        this.facturaSeleccionada = this.facturaService.getById(this.notaDebito.getFacturaAplicada().getId(), true);
    ...
    public void obtenerProvinciasPrestoServicio() throws ServiceException {
        ...
        if (this.notaDebito.getProvinciaPrestoServicio() != null && !this.tieneConvenioMultilateral(this.notaDebito.getProvinciaPrestoServicio().getNombre())) {
            this.setProvinciaPrestoServicio(this.notaDebito.getProvinciaPrestoServicio().getNombre() + " (*)");
        }
    }
    ...
    public void obtenerItems() {
        ...
        Double n = 0.0;
        for (final Linea linea : this.notaDebito.getLineas()) {
            if (linea.getTipo() == Linea.TipoLinea.ITEM) {
                ...
                if (linea.getImporte() != null) {
                    ...
                    NotaDebitoProveedorAction.log.warn((Object)("Importe de l\u00ednea Id " + linea.getId() + " nulo. Se omite."));
                }
            }
        }
    ...
    public FacturaEstado getEstadoActual() {
        try {
            return this.facturaEstadoService.getEstadoActual((Comprobante)this.notaDebito);
        }
        catch (ServiceException ex) {
            ex.printStackTrace();
            return null;
    ...
    public boolean actualizar() throws ServiceException {
        ...
        if (!this.provinciaPrestoServicio.isEmpty()) {
            ...
            this.notaDebito.setProvinciaPrestoServicio(this.provinciaService.getProvinciaByName(s));
        }
        if (this.notaDebito.getId() == 0L) {
            this.notaDebito.setLineas((List)new ArrayList());
            this.notaDebito.setEstados((List)new ArrayList());
            for (final Linea linea : this.listaLineas) {
                linea.setNumero(Long.valueOf(n));
                this.notaDebito.getLineas().add(linea);
                ++n;
            }
            for (final Linea linea2 : this.listaImpuestos) {
                linea2.setNumero(Long.valueOf(n));
                this.notaDebito.getLineas().add(linea2);
                ++n;
            }
            final List<MessageBean> actualizar = this.notaDebitoService.actualizar(this.notaDebito, (List)new ArrayList());
            ...
            for (final Linea linea3 : this.notaDebito.getLineas()) {
            ...
            this.notaDebito.getLineas().removeAll(list2);
            for (final Linea linea4 : this.listaLineas) {
                linea4.setNumero(Long.valueOf(n));
                ++n;
                if (!this.notaDebito.getLineas().contains(linea4)) {
                    this.notaDebito.getLineas().add(linea4);
                }
            }
            for (final Linea linea5 : this.listaImpuestos) {
                linea5.setNumero(Long.valueOf(n));
                ++n;
                if (!this.notaDebito.getLineas().contains(linea5)) {
                    this.notaDebito.getLineas().add(linea5);
                }
            }
            list.addAll(this.lineasToRemove);
            list.addAll(this.lineasImpuestoEliminar);
            final List<MessageBean> actualizar2 = this.notaDebitoService.actualizar(this.notaDebito, (List)list);
    ...
    private void create(final Factura facturaAplicada) throws ServiceException {
        ...
        (this.notaDebito = new NotaDebito()).setHabilitado(true);
        this.notaDebito.setEstado(EstadoFactura.INGRESADA);
        this.notaDebito.setFecha(new Date());
        this.notaDebito.setFacturaAplicada(facturaAplicada);
        this.notaDebito.setMoneda(this.facturaSeleccionada.getMoneda());
        this.notaDebito.setProveedor(this.proveedor);
        this.notaDebito.setCompania(this.facturaSeleccionada.getCompania());
        this.notaDebito.setLineas((List)new ArrayList());
        this.notaDebito.setLetra(facturaAplicada.getLetra());
    ...
    public void agregarItems() {
        for (final Map.Entry<Linea, Boolean> entry : this.lineasDisponiblesSeleccionadas.entrySet()) {
            if (entry.getValue()) {
                ...
                linea.setComprobante((Comprobante)this.notaDebito);
    ...
    public void agregarItemImpuesto() {
        ...
        try {
            this.nuevoItemImpuesto.setImpuesto(this.impuestoService.getImpuestoByName(this.notaDebito.getCompania(), this.nombreImpSeleccionado));
    ...
    public void enviar() {
        try {
            if (this.validarImportes() && this.validarAdjuntos() && this.actualizar()) {
                this.load(this.notaDebito.getId());
                this.notaDebitoService.enviar(this.notaDebito);
                Mensajes.displayFacesMessage(FacesMessage.SEVERITY_INFO, "La nota de d\u00e9bito se envi\u00f3 correctamente");
                this.volver();
            }
        }
        catch (ServiceException ex) {
    ...
    public void eliminar() {
        try {
            this.comprobanteService.eliminar((Comprobante)this.notaDebito);
        }
        catch (ServiceException ex) {
            final Iterator<String> iterator = ex.getMessages().iterator();
            while (iterator.hasNext()) {
        ...
        Mensajes.displayFacesMessage(FacesMessage.SEVERITY_INFO, "La nota de d\u00e9bito se elimin\u00f3 correctamente");
        this.volver();
    }
    ...
    public void abrirPopUpImpuesto() {
        ...
        this.nuevoItemImpuesto.setComprobante((Comprobante)this.notaDebito);
    ...
    public boolean validarAdjuntos() {
        ...
        if (this.getAdjuntos().size() == 0) {
            Mensajes.displayFacesMessage(FacesMessage.SEVERITY_ERROR, "El apartado de Adjuntos es obligatorio, debe ingresar un adjunto para poder enviar la nota de d\u00e9bito");
            b = false;
        }
        return b;
    }
    ...
    private boolean importesValidos() throws ServiceException {
        double n = 0.0;
        final double doubleValue = this.notaDebito.getImporte();
        for (final Linea linea : this.listaLineas) {
            if (linea.getImporte() != null) {
                n += linea.getImporte();
            }
    ...
    public boolean validarImportes() {
        ...
        try {
            if (!this.importesValidos()) {
                Mensajes.displayFacesMessage(FacesMessage.SEVERITY_ERROR, "El importe de la nota de d\u00e9bito debe ser igual a la suma de los importes de los \u00edtems");
                b = false;
            }
        }
        catch (ServiceException ex) {
    ...
    public void agregarAdjunto(final FileUploadEvent fileUploadEvent) {
        try {
            if (fileUploadEvent.getFile().getSize() / 1024L < 5120L) {
                ...
                this.nuevoAdjunto.setComprobante((Comprobante)this.notaDebito);
    ...
    public boolean canShowEstadoMotivo() {
        if (this.notaDebito.getEstado() == null) {
            return false;
        }
        switch (this.notaDebito.getEstado().ordinal()) {
            case 1: {
                return true;
            }
            case 2:
            case 3: {
                return this.notaDebito.getEstadoEbs() != null && this.perfilUsuario != null && this.perfilUsuario != Perfil.PROVEEDOR;
            }
            default: {
                return false;
            }
    ...
    public List<ComprobanteAdjunto> getAdjuntos() {
        if (this.adjuntos == null && this.notaDebito != null) {
            try {
                if (this.notaDebito.getId() > 0L) {
                    this.adjuntos = this.comprobanteAdjuntoService.getAdjuntos((Comprobante)this.notaDebito);
                }
            }
            catch (ServiceException ex) {
                ex.printStackTrace();
    ...
    static {
        log = Logger.getLogger(NotaDebitoProveedorAction.class);
    }
}
